...
// --- CONSTANTES GLOBALES (Extraídas del PDF) ---
...
const INITIAL_WORKOUT_PLAN = [
  { week: 1, day: 1, title: 'Semana 1 - Día 1 (Empuje)', exercises: [{ id: 'w1d1e1', name: 'Press Banca', sets: '4x8', rest: '90s', completed: false, weight: '' }, { id: 'w1d1e2', name: 'Press Militar', sets: '3x10', rest: '60s', completed: false, weight: '' }] },
  { week: 1, day: 2, title: 'Semana 1 - Día 2 (Tracción)', exercises: [{ id: 'w1d2e1', name: 'Dominadas', sets: '4xMax', rest: '120s', completed: false, weight: '' }, { id: 'w1d2e2', name: 'Remo con Barra', sets: '4x10', rest: '90s', completed: false, weight: '' }] },
];
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center">
      <div className="max-w-md w-full h-screen bg-white shadow-2xl overflow-hidden flex flex-col relative border-x border-gray-200">
              ...
              <div className="pt-4 text-left">
                ...
                <div onClick={() => setActiveTab('workout')} className={`p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between cursor-pointer active:scale-[0.97] transition-all bg-white hover:bg-gray-50 ${dailyStatus !== 'active' ? 'opacity-50 grayscale' : ''}`}>
                  <div className="flex items-center space-x-5">
                    ...
                    <div><h4 className="font-bold text-gray-800 text-sm tracking-tight">{workoutPlan[currentSessionIndex]?.title || 'Entrenamiento'}</h4><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 tracking-tighter">Toca para abrir ejercicios</p></div>
                  </div>
          ...
          {activeTab === 'nutrition' && (
            <div className="space-y-6 relative animate-in fade-in duration-500">
              ...
              <div className="flex justify-between items-center text-gray-800">
                ...
                <button onClick={() => {setImportType('nutrition'); setShowImportModal(true);}} className="p-2.5 bg-indigo-50 text-indigo-600 rounded-2xl hover:bg-indigo-100"><Upload size={20} /></button>
              </div>
              ...
              {['desayuno', 'almuerzo', 'merienda', 'cena'].map(meal => (
                <div key={meal} className="space-y-2">
                  <div className="flex justify-between items-center px-4"><h4 className="font-black text-gray-800 uppercase tracking-widest text-[10px]">{meal}</h4><span className="text-[10px] font-black text-gray-300 uppercase">{dailyLog[meal].reduce((a, b) => a + (Number(b.calories) || 0), 0)} kcal</span></div>
                  <div className="p-4 rounded-[2rem] border border-dashed border-gray-200 bg-gray-50/50 flex flex-col justify-center">
                    {dailyLog[meal].map(item => (
                      <div key={item.logId} className="flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 mb-2 shadow-sm">
                        <div className="text-left"><p className="font-bold text-xs text-gray-700">{item.label}</p><p className="text-[9px] text-gray-400 font-bold uppercase">{item.calories} kcal</p></div>
              ...
              <div className={`p-6 rounded-[2rem] border border-gray-100 bg-white shadow-sm transition-all ${alcoholLog.consumed ? 'bg-purple-50 border-purple-100' : ''}`}>
                <div className="flex items-center justify-between"><div className="flex items-center"><Wine size={18} className={`mr-2 ${alcoholLog.consumed ? 'text-purple-600' : 'text-gray-400'}`} /><span className="text-xs font-black text-gray-600 uppercase tracking-tighter">¿Hubo alcohol hoy?</span></div><button onClick={() => setAlcoholLog(p => ({...p, consumed: !p.consumed}))} className={`w-12 h-6 rounded-full relative transition-all duration-500 ${alcoholLog.consumed ? 'bg-purple-600' : 'bg-gray-200'}`}><div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-300 ${alcoholLog.consumed ? 'left-7' : 'left-1'}`} /></button></div>
                {alcoholLog.consumed && (
          ...
          {activeTab === 'workout' && (
            <div className="space-y-6 relative animate-in fade-in duration-500">
              ...
              <div className="flex justify-between items-center text-left text-gray-800">
                ...
                <button onClick={() => {setImportType('workout'); setShowImportModal(true);}} className="p-2.5 bg-indigo-50 text-indigo-600 rounded-2xl hover:bg-indigo-100"><Upload size={20} /></button>
              </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-500 text-left">
              ...
              {selectedDayData ? (
                <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm animate-in slide-in-from-bottom-6 duration-500 text-left">
                   ...
                   <div className="flex space-x-2 bg-gray-100 p-1.5 rounded-2xl mb-6"><button onClick={() => setDetailTab('nutrition')} className={`flex-1 py-3 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${detailTab === 'nutrition' ? 'bg-white text-indigo-600 shadow-md' : 'text-gray-400'}`}>Comidas</button><button onClick={() => setDetailTab('workout')} className={`flex-1 py-3 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${detailTab === 'workout' ? 'bg-white text-indigo-600 shadow-md' : 'text-gray-400'}`}>Rutina</button></div>
                  {detailTab === 'nutrition' ? (
                    ...
                    <div className="space-y-4 text-left">
                      ...
                      <div className="divide-y divide-gray-100">{selectedDayData.details?.map((ex, idx) => (<div key={idx} className="flex justify-between items-center py-4 text-xs font-black text-gray-700 tracking-tight text-left text-gray-800"><span>{ex.name}</span><span className="text-indigo-600">{ex.weight || '-'} kg</span></div>))}</div>
                    </div>
                  )}
                </div>
              ) : selectedHistoryDay && (
        ...
        {/* NAVEGACIÓN INFERIOR (TOTALMENTE FIJA Y FUERA DEL SCROLL) */}
        ...
        {showAddMealModal && (
          <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300 text-left">
            <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[80vh]">
              <div className="flex justify-between items-center mb-8 shrink-0 text-gray-800"><h3 className="text-xl font-black capitalize tracking-tight">Registrar {selectedMealType}</h3><button onClick={() => setShowAddMealModal(false)} className="text-gray-300 hover:text-gray-900"><XCircle size={32} /></button></div>
              <div className="space-y-4 overflow-y-auto pr-2 scrollbar-hide">
                ...
                {FOOD_DATABASE
                  .filter(f => (selectedMealType === 'desayuno' || selectedMealType === 'merienda') ? f.type === 'breakfast' : (f.type === 'protein' || f.type === 'carb' || f.type === 'veggie'))
                  .map(f => (<button key={f.id} onClick={() => handleAddFoodToLog(f)} className="w-full text-left p-5 border border-gray-50 rounded-[1.5rem] text-sm font-bold flex justify-between items-center hover:bg-indigo-50 transition-all text-gray-800 shadow-sm active:bg-indigo-100"><div className="flex-1 text-left"><p className="tracking-tight font-bold">{f.label}</p><p className="text-[10px] text-gray-300 font-black uppercase mt-1 tracking-widest">{f.calories} kcal • P:{f.protein}g C:{f.carbs}g</p></div><div className="bg-indigo-50 p-2 rounded-xl text-indigo-600"><Plus size={16} strokeWidth={3} /></div></button>))}
              </div>
            </div>
          </div>
        )}
        ...
        {showImportModal && (
          <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-xs rounded-[3rem] p-10 text-center shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              {importType === 'workout' && (<button onClick={downloadWorkoutTemplate} className="text-indigo-600 text-[10px] font-black mb-8 block w-full hover:underline uppercase tracking-widest flex items-center justify-center"><Download size={14} className="mr-1"/> Plantilla .CSV</button>)}
